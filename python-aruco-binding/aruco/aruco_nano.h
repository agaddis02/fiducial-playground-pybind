...
Copyright 2023 Rafael Munoz-Salinas
...
struct PerspectiveTransformer{
    ...
    cv::Point2f operator()(const cv::Point2f &point){
        double *matPtr = transformMat.ptr<double>(0);
        double xPrime = matPtr[0] * point.x + matPtr[1] * point.y + matPtr[2];
        double yPrime = matPtr[3] * point.x + matPtr[4] * point.y + matPtr[5];
        double w = matPtr[6] * point.x + matPtr[7] * point.y + matPtr[8];
        return cv::Point2f(xPrime / w,yPrime / w);
    }
    cv::Mat transformMat;
};
}
std::vector<Marker> MarkerDetector::internalDetect(const cv::Mat &inputMat, unsigned int maxAttemptsPerCandidate, std::span<const uint64_t> dictionary){
    if(maxAttemptsPerCandidate == 0) { maxAttemptsPerCandidate = 1 ;}
    cv::Mat grayMat, thresholdedMat;
    std::vector<Marker> detectedMarkers;

    if(inputMat.channels() == 3) { cv::cvtColor(inputMat,grayMat,cv::COLOR_BGR2GRAY); }
    else { grayMat = inputMat; }
    cv::adaptiveThreshold(grayMat, thresholdedMat, 255.0,cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY_INV, 7, 7);
    std::vector<std::vector<cv::Point>> foundContours;
    std::vector<cv::Point> approxContour;
    cv::RNG rng;
    cv::findContours(thresholdedMat, foundContours, cv::noArray(), cv::RETR_LIST, cv::CHAIN_APPROX_NONE);
    cv::Mat sampledMarker(8,8,CV_8UC1);
    for (unsigned int contourIdx = 0; contourIdx < foundContours.size(); contourIdx++) {
        if (50 > int(foundContours[contourIdx].size())) continue;
        cv::approxPolyDP(foundContours[contourIdx], approxContour, double(foundContours[contourIdx].size()) * 0.05, true);
        if (approxContour.size() != 4 || !cv::isContourConvex(approxContour)) continue;
        Marker marker;
        for (int pointIndex = 0; pointIndex < 4; pointIndex++) {
            marker.push_back( cv::Point2f( approxContour[pointIndex].x,approxContour[pointIndex].y));
        }
            ...
            for(int row = 0; row < sampledMarker.rows; row++) {
                for(int col = 0; col < sampledMarker.cols; col++) {
                    auto linearlySampled = uchar(
                        ...
                        linearSampleImage(
                            grayMat,
                            transformer(cv::Point2f(
                                float(col + 0.5) / float(sampledMarker.cols),
                                float(row + 0.5) /
                                    float(sampledMarker.rows)))));
                    sampledMarker.at<uchar>(row,col) = linearlySampled;
                    totalSampledValue += linearlySampled;
                }
            }
            double thresholdCutoff = double(totalSampledValue) / double(sampledMarker.cols * sampledMarker.rows);
            cv::threshold(sampledMarker,sampledMarker,thresholdCutoff,255,cv::THRESH_BINARY);
            int numRotationsNeeded = 0;
            marker.id = findMarkerId(sampledMarker, numRotationsNeeded, dictionary);
            if(marker.id == -1) { 
                continue;
            }
            std::rotate(marker.begin(),marker.begin() + 4 - numRotationsNeeded,marker.end());
        }
        if(marker.id != -1) {
    ...
    auto uniqueEnd = std::unique(detectedMarkers.begin(), detectedMarkers.end(),
        [](const Marker &first,const Marker &second){ return first.id == second.id; });
    ...
    if(detectedMarkers.size() > 0){
        ...
        cv::cornerSubPix(grayMat, pointsToSubPix, {winSize,winSize}, {-1, -1},
            { cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 12, 0.005});
        for (unsigned int markerIdx = 0; markerIdx < detectedMarkers.size(); markerIdx++) {
            for (int pointIdx = 0; pointIdx < 4; pointIdx++) {
                detectedMarkers[markerIdx][pointIdx] = pointsToSubPix[markerIdx * 4 + pointIdx];
            }
        }
    }
...
int MarkerDetector:: findMarkerId(const cv::Mat &input, int &numRotationsNeeded, std::span<const uint64_t> &dictionary){
    ...
    for(int y = 0; y < input.cols; y++) {
        if( input.at<uchar>(0,y)!=0)return -1;
        if( input.at<uchar>(input.rows-1,y)!=0)return -1;
        if( input.at<uchar>(y,0)!=0)return -1;
        if( input.at<uchar>(y,input.cols-1)!=0)return -1;
    }
...
float MarkerDetector::linearSampleImage(const cv::Mat &mat, const cv::Point2f &point){
    float pointX = int(point.x);
    float pointY = int(point.y);
    if (pointX < 0 || pointX >= mat.cols - 1 
        || pointY < 0 || pointY >= mat.rows - 1) {
        return 0;
    }
...
Marker MarkerDetector::looksLikeProcessMarker(const Marker &marker) {
    ...
    double e = (a * d) - (b * c);
    if (e < 0.0) { std::swap(temp[1], temp[3]); }
    return temp;
}
...
std::pair<cv::Mat,cv::Mat> Marker::estimatePose(cv::Mat cameraMatrix, cv::Mat distCoeffs, double markerSize) const {
    std::vector<cv::Point3d> markerCorners= {
        { -markerSize / 2.f, markerSize / 2.f, 0.f }, { markerSize / 2.f, markerSize / 2.f, 0.f },
        { markerSize / 2.f, -markerSize / 2.f, 0.f }, { -markerSize / 2.f, -markerSize / 2.f, 0.f }};
    cv::Mat Rvec, Tvec;
...
void Marker::draw(cv::Mat &in, const cv::Scalar color) const{
    ...
    cv::rectangle(in, (*this)[0] - p2, (*this)[0] + p2, {0, 0, 255, 255}, -1);
    cv::rectangle(in, (*this)[1] - p2, (*this)[1] + p2, {0, 255, 0, 255}, lineWidth);
    cv::rectangle(in, (*this)[2] - p2, (*this)[2] + p2, {255, 0, 0, 255}, lineWidth);
    cv::Point2f cent(0, 0);
